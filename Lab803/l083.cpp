...
void makeVideo(double scale, Mat& coordinates, vector<vector<int>>& edges, int rows, int cols, bool writeCoordinates, vector<Mat>& vidFrames, Mat &a, Mat &normal, Mat &eyepos, bool genrandom) {
	...
	vector<Scalar> vertexColors = vector<Scalar>();
	srand(time(NULL));
	for (int i = 0; i < coordinates.cols; i++) {
		int b = rand() % 256;
		int g = rand() % 256;
		int r = rand() % 256;
		vertexColors.push_back(Scalar(b, g, r));
	}
	...
	for (int i = 0; i < transformationCoords.size(); i++) {
		...
		for (int i = 0; i < edges.size(); i++) {
			...
			double eyedot = normal.dot(eyepos - a);
			if (dot1*eyedot <= 0 && dot2*eyedot <= 0) {
			...
			else if (dot1*eyedot > 0 and dot2*eyedot > 0) {
				//do nothing
			}
			else {
				Mat left, right;
				//left = opposite side as eye
				//right = same side as eye
				bool isFirst = true;
				if (dot1*eyedot < 0 && dot2*eyedot > 0) {
					left = v1;
					right = v2;
				...
				Mat lr = left - right;
				Mat ar = a - right;
				double t = ar.dot(normal) / lr.dot(normal);
				Mat newright = t * lr + right;

				int x1 = cvRound(projected.at<double>(0, e1)) + (cols / 2);
				int y1 = (rows / 2) - cvRound(projected.at<double>(1, e1));
				if (!isFirst) {
					x1 = cvRound(projected.at<double>(0, e2)) + (cols / 2);
					y1 = (rows / 2) - cvRound(projected.at<double>(1, e2));
				}
					
				Mat p2;

				projectPoint(newright, p2, a, normal, right, w1, w2, p0);

				int x2 = cvRound(p2.at<double>(0, 0)) + cols / 2;
				int y2 = cvRound(rows / 2 - p2.at<double>(1, 0));
		...
		for (int p = 0; p < result.cols; p++) {
			...
			if (normal.dot(result(Range(0, 3), Range(p, p + 1))-a) < 0) {
				circle(frame, Point(xval, yval), 5, vertexColors[p], -1);
			}
