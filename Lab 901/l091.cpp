...
void drawFigure(Mat& frame, vector<Point2f> imagePoints, vector<vector<int>> edges) {
	for (int i = 0; i < edges.size(); i++) {
		int edge1 = edges[i][0];
		int edge2 = edges[i][1];
		line(frame, imagePoints[edge1], imagePoints[edge2], Scalar(255, 128, 128),8);
		circle(frame, imagePoints[edge1], 10, Scalar(0, 0, 255),-1);
		circle(frame, imagePoints[edge2], 10, Scalar(0, 0, 255), -1);
	}
}
/**
...
void placeObject(vector<Mat>& vidFrames, vector<Mat>& outputFrames, Mat& coordinates, vector<vector<int>> edges) {
	...
	for (int c = 0; c < vidFrames.size(); c+=20) {
	...
	bool prevfound = true;
	for (int i = 0; i < vidFrames.size(); i++) {
		...
		if (i%50==0) { //reasses the corners every 20 frames
			//use the corners detected from findChessBoard to calibrate lkoptical flow
			vector<Point2f> candcorners;
			bool found = findChessboardCorners(gray, Size(boardCols, boardRows), candcorners, CALIB_CB_ADAPTIVE_THRESH + CALIB_CB_NORMALIZE_IMAGE + CALIB_CB_FAST_CHECK);
			if (found) {
				TermCriteria criteria = TermCriteria(TermCriteria::EPS + TermCriteria::COUNT, 30, 0.001);
				cornerSubPix(gray, candcorners, Size(11, 11), Size(-1, -1), criteria);
				solvePnP(objectPoints, candcorners, cameraMatrix, dist, rvecs, tvecs);
				...
				prevfeaturePoints = candcorners; //update prevfeaturePoints with good 2dcorners from this iteration
				prevfound = found;
				continue;
			}
		//else {
		...
		calcOpticalFlowPyrLK(oldGray, gray, prevfeaturePoints, cornerCandidates, status, err, Size(15, 15), 2, criteria, 1.0E-4);
		...
		/*vector<Point2f> 
		if (count < 49 && findChessboardCorners()) {
