...
*/
    ...
    */
    ...
    void removeNonMax(unordered_set<Coordinate, Coordinate::HashFunc> &centers){
        for(auto it=centers.begin(); it!=centers.end();){
            ...
            for(size_t d=0; d < directions.size(); d++){
                int compR = r+ directions[d][0];
                int compC = c+directions[d][1];
                if(compR >= 0 && compC >= 0 && compR < grid.size() && compC < grid[0].size()){
                    if(grid[r][c] < grid[compR][compC]){
                        flag=true;
                        break;
                    }
                }
...
void part2(const int &argc, char **&argv)
{
    ...
    votes.removeNonMax(centers);
    cout << "\nlen of centers after removal " << centers.size() << endl;
