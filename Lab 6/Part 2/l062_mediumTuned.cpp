    ...
    void cleanUp(int kernelsize, double percent, int threshold, unordered_set<Coordinate, Coordinate::HashFunc> &centers, bool override = false)
    {
        ...
        for (size_t i = kernelsize; i < grid.size() - kernelsize; i += kernelsize)
        {
            for (size_t j = kernelsize; j < grid[0].size() - kernelsize; j += kernelsize)
            {
                ...
                int q1threshold = maxIntensity * 0.27;
...
void eliminateFalseCircles(Grid &gr, vector<Circle> &circles, int distanceThreshold, vector<Circle> &finalCircles)
{
    ...
    for (size_t i = 0; i < circles.size(); i++)
    {
        ...
        if (idxToRemove.find(i) == idxToRemove.end())
        {
            ...
            for (size_t j = i + 1; j < circles.size(); j++)
            {
                ...
                int midpoint = (c1.getRadius() + c2.getRadius())/2;
                if (dist < distsqred)
                {
                    //if(abs(c1.getRadius()-c2.getRadius()) < 0.1*midpoint){
 ...
 * @para
 */
void identifyCoins(Grid &gr, vector<Circle> &finalCircles, int penny, int nickel, int quarter, int halfdollar, vector<int> &coinTypes){
...
//    cout << "Image max Intensity " << gr.getMaxIntensity() << endl;
    for(int idx=0; idx < finalCircles.size(); idx++){
        Circle &circle = finalCircles[idx];
...
{   
    ...
    */
    ...
    eliminateFalseCircles(*gr, circles, 80, finalCircles);
    ...
    for(int i=0; i < coinTypes.size(); i++){
        Coordinate center = finalCircles[i].getCenter();
        int radius = finalCircles[i].getRadius();
        if(coinTypes[i]==0){
