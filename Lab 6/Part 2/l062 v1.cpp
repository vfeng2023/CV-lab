...
void medianBlur(Grid &gr,int kernelsize){
...
void findCircles(unordered_set<Coordinate, Coordinate::HashFunc> centers, Grid &edges,vector<Circle> &circs)
{
    int minRad = max(60, edges.getHeight() / 20);
    int maxRad = max(120, edges.getHeight() / 10);
...
void part2(const int &argc, char **&argv)
{
    ...
    string fv = "imagev.ppm";
    string fcc = "imageCC.ppm";
    int thresholdCount = 15;
    // string imageName = "single_coin.ppm";
    // int low = 100;
    // int high = 150;
    // string fg = "imageg.ppm";
    // string of = "image1.ppm";
    ...
    findCircles(centers,*edgebinary,circles);
    for (const Circle &c : circles)
    {
        ...
        {
            edgebinary->drawCircle(IntegerPoint(center.getX(), center.getY()), i, 0, edgebinary->getMaxIntensity(), 0);
        }
    }
    cout << "len of circles " << circles.size() << endl;
    edgebinary->toColorPPM("imageCircles.ppm");
    delete gr;
    delete edgebinary;
}
