...
Grid *readImage(string filename)
{
    ...
    */
    ...
    CIRCLE_RADIUS = min(width / 10, height / 10);
    imgFile.close();
    return gr;
}
...
public:
    ...
    Circle(Coordinate &cent, int rad)
    {
        center = cent;
        radius = rad;
    }
...
void findCircles(unordered_set<Coordinate, Coordinate::HashFunc> centers, Grid &edges)
{
    int minRad = min(60, edges.getHeight() / 20);
    ...
    for (const Coordinate &cent : centers)
    {
        ...
        for (int radius = minRad; radius <= maxRad; radius++)
        {
            ...
            for (x = 0; x <= y; x++)
            {
                ...
                // shift center from given algorithm
                if (edges.getPixel(x + xCC, y + yCC) == 1)
                {
                    counts[countsIdx] += 1;
                }
                if (edges.getPixel(x + xCC, -y + yCC) == 1)
                ...
                if (edges.getPixel(y + xCC, x + yCC) == 1)
                {
                    counts[countsIdx] += 1;
                }
                if (edges.getPixel(y + xCC, -x + yCC) == 1)
