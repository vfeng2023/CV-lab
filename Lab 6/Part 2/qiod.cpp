...
public:
    ...
    Grid(const Grid &gr)
    {
        grid = vector<vector<Pixel>>(gr.grid);
        width = gr.width;
        height = gr.height;
...
*/
    ...
    */
                ...
                if (grid[r][c] != 0)
                {
                    ...
                    for (int compR = r - halfkernelsize; compR < r + halfkernelsize; compR++)
                    {
                        for (int compC = c - halfkernelsize; compC < c + halfkernelsize; compC++)
                        {
                            ...
                            if (compR >= 0 && compC >= 0 && compR < int(grid.size()) && compC < int(grid[0].size()))
                ...
                for (int y = startr; y < startr + filtersize; y++)
                {
                    for (int x = startc; x < startc + filtersize; x++)
                    {
                        ...
                        else
                        {
                            ...
                            *
                            */
                            if (y < int(grid.size()) / 2)
                            {
                                if (x < int(grid[0].size()) / 2)
                                {
    ...
    void top10(unordered_set<Coordinate, Coordinate::HashFunc> &centers, int filtersize)
    {
        ...
        for (int r = filtersize; r < int(grid.size()) - filtersize; r += filtersize)
        {
            for (int c = filtersize; c < (int)grid[0].size() - filtersize; c += filtersize)
            {
                int localMax = 0;
...
void findCircles(Grid &image, unordered_set<Coordinate, Coordinate::HashFunc> centers, Grid &edges, vector<Circle> &circs, int percentageThreshold)
{
...
void part2(const int &argc, char **&argv)
{
    ...
    findCircles(*gr, centers, *edgebinary, circles,tCircle);
    cout << "len of circles " << circles.size() << endl;
    //eliminate False Circles
    vector<Circle> finalCircles = vector<Circle>();
    eliminateFalseCircles(*gr, circles, minCenterDist, finalCircles);
    ...
    for(size_t i=0; i < coinTypes.size(); i++){
        ...
        if(coinTypes[i]==0){
            ...
            for (int r = radius - 1; r <= radius + 1; r++)
            {
                grcopy->drawCircle(IntegerPoint(center.getX(), center.getY()), r, gr->getMaxIntensity()*2/3, 0, gr->getMaxIntensity());
            }
