...
public:
    ...
    Circle(const Coordinate &cent, int rad,double frac)
    {
        center = cent;
        radius = rad;
        matchFraction = frac;
...
{
    int minRad = min(60, edges.getHeight() / 50);
    ...
    for (const Coordinate &cent : centers)
    {
        ...
        for(int r=minRad+1; r < maxRad; r+=1 ){
            ...
            if(bincount > decimalthreshold * edgcnts[idx]){
                ...
                circs.push_back(Circle(cent,r,matchFrac));
...
void eliminateFalseCircles(Grid &gr, vector<Circle> &circles,int halfkernelsize,vector<Circle> &finalCircles){
    //keep centers that have the highest intensity 

    //if a center has circles of multiple radii associated with it, keep only the radii that have the highest match percentages (top 90%)
    // require:
        //center: circle map
    unordered_map<Coordinate,vector<Circle>, Coordinate::HashFunc> circle_map = unordered_map<Coordinate,vector<Circle>, Coordinate::HashFunc>();
    for(const Circle &circ:circles){
        if(circle_map.find(circ.getCenter())==circle_map.end()){
            circle_map[circ.getCenter()] = vector<Circle>();
        }
            circle_map[circ.getCenter()].push_back(circ);
        
    }
    cout << "line 1446\n";
    for(auto it=circle_map.begin();it!=circle_map.end(); it++){
        vector<Circle> &arr = it->second;
        std::sort(arr.begin(),arr.end(),Circle::compareMatchFraction);

    }
    //for each kernel x kernel window region
    //for each center in the kernel region
            // keep the center in the top 10 percentile for match percentages
 cout << "line 1455 \n"; 
    for(int r=halfkernelsize; r < gr.getHeight()-halfkernelsize; r+=halfkernelsize){
        for(int c=halfkernelsize; c < gr.getWidth()-halfkernelsize; c+=halfkernelsize){
            vector<double> localMatches = vector<double>();
            vector<Coordinate> localcenters = vector<Coordinate>();
            for(int y=r-halfkernelsize; y < r+halfkernelsize; y++){
                for(int x=c-halfkernelsize; x < c+halfkernelsize; x++){
                    if(gr.getCenter(y,x)){
                        localMatches.push_back((circle_map[Coordinate(y,x)])[0].getMatchFraction());
                        localcenters.push_back(Coordinate(y,x));
                    }
                }
            }
            // cout << "line 1468\n";
            std::sort(localMatches.begin(),localMatches.end(),std::greater<double>());
            if(localMatches.size()<=2) continue;
            //get rid of centers that are not of the most matches
            int idx = max(1,int(localMatches.size()*0.1));
            double localthreshold = localMatches[idx]*0.9;
            for(const Coordinate &cent:localcenters){
                if(circle_map[cent][0].getMatchFraction() < localthreshold){
                    circle_map.erase(cent);
                }
            }

        }
    }
    //eliminate the circles which have low match fractions
    for(auto it=circle_map.begin();it!=circle_map.end(); it++){
        vector<Circle> &arr = it->second;
        //std::sort(arr.begin(),arr.end(),Circle::compareMatchFraction);
        for(int i=0; i < arr.size(); i++){
            finalCircles.push_back(arr[i]);
        }
