...
void part2(const int &argc, char **&argv)
{
    ...
    votes.cleanUp(5, 0.1, thresholdCount, centers, false);
    votes.top10(centers, 5);
    cout << "len of centers: " << centers.size() << endl;
    //votes.maxFilter(1);
    //votes.gaussVotesBlur(1,2);
    //votes.findCenterCandidates(centers, 1);
    
    //votes.toPPM(votes.getMaxIntensity(), "imagePeaks.ppm");
    
    
    //cout << "\nlen of centers after cleanup: " << centers.size() << endl;
