...
private:
    ...
    vector<vector<int>> transformedGrid;
    int height;
    int width;
    int maxIntensity;
...
*/
    ...
    */
    ...
    void medFilter(int halfsize)
    {
        ...
        transformedGrid = medianFiltered;
    }
    /**
                    ...
                    for (int compR = r - halfkernelsize; compR < r + halfkernelsize; compR++)
                    {
                        for (int compC = c - halfkernelsize; compC < c + halfkernelsize; compC++)
                        {
                            ...
                            if (compR >= 0 && compC >= 0 && compR < grid.size() && compC < grid[0].size())
                                localmax = max(localmax, transformedGrid[compR][compC]);
                        }
                    }
                    if (localmax == transformedGrid[r][c])
                    {
