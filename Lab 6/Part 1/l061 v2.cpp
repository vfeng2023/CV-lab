...
public:
    ...
    void voteAlongLine(int x, int y, double angle){
        ...
        if(abs(abs(angle)-90) < ERROR){ // if vertical line
            ...
            x0 = (0-dbly)/slope + dblx;
            x1 = (1-dbly)/slope + dblx;
            y0 = slope *(0-dblx) + dbly;
            
            y1 = slope *(1-dblx) + dbly;
            IntegerPoint i0;
            IntegerPoint i1;
            bool foundFirst = false; 
            if(x0 >= -ERROR && x0 <= 1+ERROR){
                i0 = DoublePoint(x0,0).scaleToInt(height,width);
                foundFirst = true;
            }
            if(x1 >= -ERROR && x1 <= 1+ERROR){
                IntegerPoint pt = DoublePoint(x1, 1).scaleToInt(height,width);
                if(foundFirst){
                    i1 = pt;
                }else{
                    i0 = pt;
            ...
            if(y0 >=  -ERROR && y0 <= 1+ERROR){
                IntegerPoint pt = DoublePoint(0, y0).scaleToInt(height,width);
                if(foundFirst){
                    i1 = pt;
                }else{
                    i0 = pt;
            ...
            if(y1 >= -ERROR && y1 <= 1+ERROR){
                IntegerPoint pt = DoublePoint(1, y1).scaleToInt(height,width);
                if(foundFirst){
                    i1 = pt;
                }else{
                    i0 = pt;
...
void vote(Grid &edges, VoteMatrix &votes, vector<vector<double>> angles){
    ...
    for(int r=0; r < edges.getHeight(); r++){
        for(int c=0; c < edges.getWidth(); c++){
            ...
            //     cout << "Row, COl" << r << " " << c << " angle: " << angles[r][c]<<endl; 
            //     return;
            // }
        }
    }
...
void part1(const int &argc, char **&argv)
{
    ...
    //votes.voteAlongLine(120,120,135);
    ...
    votes.toPPM(maxIntensity, "imagev.ppm");
