...
public:
    ...
    void setCenter(int r, int c,bool val=true){
        grid[r][c].setCenter(val);
    }
    bool getCenter(int r, int c){
        return grid[r][c].center();
    }
    int getHeight()
    {
        return height;
...
*/
    ...
    */
    ...
    /**
     * @brief Kernelsize should be an odd number
     * 
     * @param gr 
     * @param kernelsize 
     */
    void suppressNoise(Grid &gr,int kernelsize,double percent){
        int k = kernelsize/2;
        for(int r=k; r < gr.getHeight()-k; r++){
            for(int c=k; c < gr.getWidth()-k; c++){
                int startrow = r-k;
                int startcol = c-k;
                unordered_map<Coordinate,int,Coordinate::HashFunc> coordToIndensity = unordered_map<Coordinate,int,Coordinate::HashFunc>();
                vector<int> intensities = vector<int>();
                for(int i=startrow; i < startrow+kernelsize; i++){
                    for(int j=startcol; j < startcol+kernelsize; j++){
                        if(gr.getCenter(i,j)){
                            coordToIndensity[Coordinate(i,j)] = grid[i][j];
                            intensities.push_back(grid[i][j]);
                        }
                    }
                }
                if(intensities.size() <= 3) continue;
                std::sort(intensities.begin(),intensities.end(),greater<int>());
                int maxCount = (int)(percent*intensities.size()+1);
                for(auto it = coordToIndensity.begin(); it!=coordToIndensity.end();it++){
                    Coordinate coord = it->first;
                    if(it->second < intensities[maxCount]){
                        gr.setCenter(coord.getX(),coord.getY(),false);
                    }
    ...
    void determineCenters(Grid &gr, int threshold){
        for(int r=0; r < grid.size(); r++){
            for(int c = 0; c < grid[0].size(); c++){
                if(grid[r][c] > threshold){
                    gr.setCenter(r,c,true);
                }
            }
        }
    }
...
void part1(const int &argc, char **&argv)
{
    ...
    int low = 75;
    int high = 100;
    string fg = "cpgray.ppm";
    string of = "cpimage1.ppm";
    string f2 = "cpimage2.ppm";
    string ff = "cpimagef.ppm";
    ...
    std::cout << "-F " << imageName << "-L " << low << " -H " << high << " -TC "<< thresholdCount << endl;
    // read in the image
    ...
    //     }
    // }
    votes.toPPM(maxIntensity, "cpv.ppm");
    std::cout << "Votes image created, max =" << maxIntensity << endl;
    votes.determineCenters(*gr,thresholdCount);
    votes.suppressNoise(*gr,5,0.1);
