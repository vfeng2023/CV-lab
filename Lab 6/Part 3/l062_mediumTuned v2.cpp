...
public:
    ...
    int getRed(int r,int c){
        return grid[r][c].getRed();
    }
    int getGreen(int r,int c){
        return grid[r][c].getGreen();
    }
    int getBlue(int r,int c){
        return grid[r][c].getBlue();
    }
    int getWidth()
    {
        return width;
...
*/
    ...
    */
    ...
    {
        vector<vector<int>> medianFiltered = vector<vector<int>>(height, vector<int>(width, 0));
        ...
        for (int r = halfsize; r < height - halfsize; r++)
        {
            for (int c = halfsize; c < width - halfsize; c++)
            {
                ...
                medianFiltered[r][c] = vals[vals.size() / 2];
            }
        }
        grid = medianFiltered;
    }
        ...
        for (int r = halfsize; r < height - halfsize; r++)
        {
            for (int c = halfsize; c < width - halfsize; c++)
            {
                ...
                for (int y = startR; y <= r + halfsize; y++)
                {
                    for (int x = startC; x <= c + halfsize; x++)
                    {
                        maxval = max(maxval, grid[y][x]);
                    }
                }
                // std::sort(vals.begin(), vals.end());
                medianFiltered[r][c] = maxval;
            }
        }
        grid = medianFiltered;
    }
        ...
        for (int r = k; r < height - k; r++)
        {
            for (int c = k; c < width - k; c++)
            {
                ...
                for (size_t i = 0; i < filter.size(); i++)
                {
                    for (size_t j = 0; j < filter.size(); j++)
                    {
                        double temp = grid[startrow + i][startcol + j] * filter[i][j];
                        val += temp;
                    }
                }
                grid[r][c] = val;
    ...
    void findCenterCandidates(unordered_set<Coordinate, Coordinate::HashFunc> &centers, int halfkernelsize)
    {
        for (int r = 0; r < grid.size(); r++)
        {
            for (int c = 0; c < grid[0].size(); c++)
            {
                ...
                if (grid[r][c] != 0)
                {
                    int localmax = grid[r][c];
                    for (int compR = r - halfkernelsize; compR < r + halfkernelsize; compR++)
                    {
                        for (int compC = c - halfkernelsize; compC < c + halfkernelsize; compC++)
                        {

                            if (compR >= 0 && compC >= 0 && compR < grid.size() && compC < grid[0].size())
                                localmax = max(localmax, grid[compR][compC]);
                        }
                    }
                    if (localmax == grid[r][c])
                    {
    ...
    void cleanUp(int kernelsize, double percent, int threshold, unordered_set<Coordinate, Coordinate::HashFunc> &centers, bool override = false)
    {
        ...
        for (size_t i = kernelsize; i < grid.size() - kernelsize; i += kernelsize)
        {
            for (size_t j = kernelsize; j < grid[0].size() - kernelsize; j += kernelsize)
            {
                ...
                int q1threshold = maxIntensity * 0.14;
                int q2threshold = maxIntensity * 0.27;
                int q3threshold = maxIntensity * 0.30;
                int q4threshold = maxIntensity * 0.30;
    ...
    void updateMaxIntensity()
    {
        for (size_t i = 0; i < grid.size(); i++)
        {
            for (size_t j = 0; j < grid[0].size(); j++)
            {
                maxIntensity = max(maxIntensity, grid[i][j]);
            }
        }
    }
    void save(){
...
void findCircles(Grid &image, unordered_set<Coordinate, Coordinate::HashFunc> centers, Grid &edges, list<Circle> &circs)
{
...
void eliminateFalseCircles(Grid &gr, list<Circle> &circles, int distanceThreshold, vector<Circle> &finalCircles)
{
 ...
 * @para
 */
void identifyCoins(Grid &gr, list<Circle> &finalCircles, int penny, int nickel, int quarter, int halfdollar, vector<int> &coinTypes){
...
//    cout << "Image max Intensity " << gr.getMaxIntensity() << endl;
    for(auto it=finalCircles.begin(); it!=finalCircles.end();++it){
        Circle &circle = *it;
        ...
        // if(hue < 0){
        //     hue += ceil(abs(hue*1.0/360)) * 360;
        // }
        // hue %=360;
        //cout << idx << "hue  " << hue << "value " << value << "saturation " << saturation << endl;
        //cout << idx << " rgb" << red << " " << green << " " << blue << endl;
        ...
        */
        if(abs(radius-penny) < 0.1*penny){
            ...
            if(hue >= -40 && hue <= 45){
                ...
                if(abs(radius-nickel) < 0.1*nickel){
                    coinTypes.push_back(1);
                }else{
                    if(abs(radius-penny) < 0.05*penny){
                        coinTypes.push_back(0);
                    }else{
        ...
        //if radius is within 10% of nickel radius:
            //classify as nickel
        else if(abs(radius-nickel) < 0.1*nickel){
            if(abs(radius-quarter) < 0.1*quarter){
        ...
        //if radius within 15 of quarter radius
            ...
            //otherwise:
                //classify as quarter
        else if(abs(radius-quarter) < 0.15*quarter){
            if(hue <=50 && hue >=40){
...
{   
    ...
    /*
    For coinsHarder.ppm, the optimal values are:
        q1 75 150 
        q2 75 150
        q3 200 250
        q4 124 250
    ...
    */
    ...
    string imageName = "coinsHarder.ppm";
    // int low = 50;
    // int high = 75;
    // string fg = "imageg.ppm";
    // string of = "image1.ppm";
    ...
    Grid *edgebinary = sobelOperatorPt2(gr, low, high, of, f2, ff, angles,thresholdOverride);
    ...
    vector<Circle> dummy = vector<Circle>();
    eliminateFalseCircles(*gr, circles, 80, dummy);

    list<Circle> &finalCircles = circles;
    //identify the coin types
    vector<int> coinTypes = vector<int>();
    identifyCoins(*gr,finalCircles,87,98,112,141,coinTypes);
    ...
    gr->toColorPPM("imageCircles.ppm");
     ...
     /*
        * Representation of coins:
        ...
        5 - dollar
        */
    auto cit = finalCircles.begin();
    for(int i=0; i < coinTypes.size(); i++){
        Coordinate center = cit->getCenter();
        int radius = cit->getRadius();
        if(coinTypes[i]==0){
            for (int r = radius - 1; r <= radius + 1; r++)
            {
                gr->drawCircle(IntegerPoint(center.getX(), center.getY()), r, gr->getMaxIntensity(), 0, 0);
            }
        }else if(coinTypes[i]==1){
            for (int r = radius - 1; r <= radius + 1; r++)
            {
                gr->drawCircle(IntegerPoint(center.getX(), center.getY()), r, gr->getMaxIntensity(), gr->getMaxIntensity(), 0);
            }
        }else if(coinTypes[i]==2){
            for (int r = radius - 1; r <= radius + 1; r++)
            {
                gr->drawCircle(IntegerPoint(center.getX(), center.getY()), r, 0, 0, gr->getMaxIntensity());
            }
        }else if(coinTypes[i]==3){
            for (int r = radius - 1; r <= radius + 1; r++)
            {
                gr->drawCircle(IntegerPoint(center.getX(), center.getY()), r, gr->getMaxIntensity()*2/3, 0, gr->getMaxIntensity());
            }
        }else if(coinTypes[i]==4){
            for (int r = radius - 1; r <= radius + 1; r++)
            {
                gr->drawCircle(IntegerPoint(center.getX(), center.getY()), r, 0, gr->getMaxIntensity(), 0);
            }
        }else if(coinTypes[i]==5){
            for (int r = radius - 1; r <= radius + 1; r++)
            {
                gr->drawCircle(IntegerPoint(center.getX(), center.getY()), r, gr->getMaxIntensity(), 0, gr->getMaxIntensity()/2);
            }
        }
        cit++;
    }
    gr->toColorPPM("imageCoins.ppm");
    summary(coinTypes,"results.txt");
    delete gr;
    delete edgebinary;
}
int main(int argc, char **argv)
{
    ...
    //Pixel result = rgbtohsv(33,52,59, 255);
    //cout << result.getRed() << " " << result.getGreen() << " " << result.getBlue() << endl;
    // int hue = result.getRed();
    // if(hue < 0){
    //         hue += ceil(abs(hue*1.0/360)) * 360;
    //     }
