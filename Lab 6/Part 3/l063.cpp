...
public:
    ...
    void voteAlongLine(int x, int y, double angle)
    {
        ...
        { // if vertical line
            //x0 = dblx;
            //x1 = dblx;
            IntegerPoint i0 = IntegerPoint(x, y - CIRCLE_RADIUS); // DoublePoint(x0, 0).scaleToInt(height, width);
            // std::cout << "i0" << i0.toString() << "\n";
            IntegerPoint i1 = IntegerPoint(x, y + CIRCLE_RADIUS); // DoublePoint(x1, 1).scaleToInt(height, width);
            // std::cout << "i0: " << i0.toString() << "\n";
        ...
        { // if horizontal line
            //y0 = dbly;
            //y1 = dbly;
            IntegerPoint i0 = IntegerPoint(x - CIRCLE_RADIUS, y); // DoublePoint(0, y0).scaleToInt(height, width);
            // std::cout << "i0" << i0.toString() << "\n";
            IntegerPoint i1 = IntegerPoint(x + CIRCLE_RADIUS, y);
            // std::cout << "i0: " << i0.toString() << "\n";
            // std::cout << "i1 " << i1.toString() << endl;
            drawLine(i0, i1);
        }
...
void eliminateFalseCircles(Grid &gr, list<Circle> &circles, int distanceThreshold)
{
    ...
    for (auto it = circles.begin(); it != circles.end(); it++)
    {
        for (auto it2 = next(it); it2 != circles.end(); it2++)
        {
            Circle &c1 = *it;
            Circle &c2 = *it2;
            double minFrac = min(c1.getMatchFraction(),c2.getMatchFraction());
            double maxFrac = max(c1.getMatchFraction(),c2.getMatchFraction());
            if(abs(minFrac-maxFrac) < 0.25*(maxFrac+minFrac)){
                ...
                int dist = max(c1.getRadius(),c2.getRadius());

                if (c1.squareddistanceToCenter(c2) < dist*dist)
                {   
                
                    // remove the one with higher match fraction
                    if (100*c1.getMatchFraction()/c1.getRadius() < 100*c2.getMatchFraction()/c2.getRadius())
                    {
                        it = circles.erase(it);
                        it--;
                        break;
 ...
 * @para
 */
void identifyCoins(Grid &gr, list<Circle> &finalCircles, int penny, int nickel, int quarter, int dollar, int halfdollar, vector<int> &coinTypes, bool ishard)
{
    ...
    for (size_t idx = 0; idx < finalCircles.size(); idx++)
    {
       ...
       */
        if (!ishard)
        {
            ...
            {
                if ((hue <= 40 && hue >= 30) && abs(radius - dollar) < 0.1 * dollar)
                {
        ...
        {
            //hard image has no quarters
            int dime = 77;
            quarter = 116;
            if(abs(radius-dime) < 0.1*dime){
            ...
            else if (abs(radius - penny) < 0.1 * penny)
            {
                // if its bronze -- classify as penny
                ...
                {
                    if (abs(radius - nickel) < 0.10 * nickel)
                    {
                        coinTypes.push_back(1);
                    }
                    else if (abs(radius - dime) < 0.1 * dime)
                    {
                        coinTypes.push_back(2);
                    }
                }
            ...
            // if radius is within 10% of the half-dollar radius
            // classify as halfdollar
            else if (abs(radius - halfdollar) < 0.10 * halfdollar)
            {
                coinTypes.push_back(5);
            }
            // add -1 if coin cannot be identified
...
void part3(int argc, char **argv)
{
    ...
    */
    ...
    identifyCoins(*gr, circles, 87, 93, 112, 132, 168, coinTypes, hardimage);
