...
*/
    ...
    */
    ...
    void cleanUp(int kernelsize, double percent, int threshold, unordered_set<Coordinate, Coordinate::HashFunc> &centers, bool override = false)
    {
        ...
        for (int i = kernelsize; i < int(grid.size() - kernelsize); i += kernelsize)
        {
            for (int j = kernelsize; j < int(grid[0].size() - kernelsize); j += kernelsize)
            {
...
void findCircles(Grid &image, unordered_set<Coordinate, Coordinate::HashFunc> centers, Grid &edges, list<Circle> &circs, int percentageThreshold, bool override = false)
{
    ...
    for (size_t idx = 0; idx < finalCircles.size(); idx++)
    {
       ...
       */
        if (abs(radius - penny) < 0.10 * penny)
        {
            ...
            // else:
            // classify as dime
            if ((hue >= -40 && hue <= 50))//||abs(radius - penny) < 0.05 * penny)
            {
...
{   
    ...
    */
    ...
    findCircles(*gr, centers, *edgebinary, circles, tCircle,tCircleOverride);
