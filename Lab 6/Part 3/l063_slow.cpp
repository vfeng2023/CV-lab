...
public:
    ...
    {
        vector<vector<Pixel>> medianFiltered = vector<vector<Pixel>>(grid);
        ...
        for (int r = halfsize; r < height - halfsize; r++)
        {
            for (int c = halfsize; c < width - halfsize; c++)
            {
                ...
                int maxval = grid[r][c].getGray();
                for (int y = startR; y <= r + halfsize; y++)
                {
                    for (int x = startC; x <= c + halfsize; x++)
                    {
                        maxval = max(maxval, grid[y][x].getGray());
                    }
                }
                // std::sort(vals.begin(), vals.end());
                medianFiltered[r][c].setGray(maxval);
            }
        }
        grid = medianFiltered;
    }
        ...
        for (int r = halfsize; r < height - halfsize; r++)
        {
            for (int c = halfsize; c < width - halfsize; c++)
            {
                ...
                for (int y = startR; y <= r + halfsize; y++)
                {
                    for (int x = startC; x <= c + halfsize; x++)
                    {
                        maxval = min(maxval, grid[y][x].getGray());
                    }
                }
                // std::sort(vals.begin(), vals.end());
                medianFiltered[r][c].setGray(maxval);
        ...
        for (int r = halfsize; r < height - halfsize; r++)
        {
            for (int c = halfsize; c < width - halfsize; c++)
            {
                ...
                for (int y = startR; y <= r + halfsize; y++)
                {
                    for (int x = startC; x <= c + halfsize; x++)
                    {
                        maxval += grid[y][x].getGray();
                        count += 1;
                    }
                }
                // std::sort(vals.begin(), vals.end());
                medianFiltered[r][c].setGray(maxval/count);
            }
        }
        grid = medianFiltered;
    }
...
void part3(int argc, char** argv){
    ...
    */
    string imageName = "coinsHardest.ppm";
    int low = 50;
    int high = 75;
