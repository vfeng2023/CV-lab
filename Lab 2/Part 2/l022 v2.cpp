...
public:
    ...
    */
    void drawLine(DoublePoint &d1, DoublePoint &d2)
    {
        ...
        cout << "\nslope " << slope << endl;
        double y0 = slope * (-1.0 * d1.getX()) + d1.getY();
        double y1 = slope * (1.0 - d1.getX()) + d1.getY();
...
*/
    ...
    */
    void toPPM(string filename = "triangle.ppm")
    {
        ofstream myfile;
...
public:
    ...
    */
    void drawCircle(Grid &gr, int color = 0)
    {
        int size = gr.getSize();
...
void highLightPoint(Grid &gr, DoublePoint &p, int rad, int color = 2)
{
    ...
    c.drawCircle(gr, color);
}
/*
    Returns array containing the vertices of the square given four points random points.
    returned vector is of length 4
*/
vector<DoublePoint> calculateLines(DoublePoint &a, DoublePoint &b, DoublePoint &c, DoublePoint &d, int dir = 1)
{
    ...
    // highLightPoint(gr,a,3,0); //black
    // highLightPoint(gr,b,3,2); //red
    // highLightPoint(gr,c,3,3); //green
    // highLightPoint(gr,d,3,4); //blue
    // highLightPoint(gr,e,3,2);
    // the four sides of the square(black)

    // linede.draw(gr);
    // sideade.draw(gr);
    // sidebac.draw(gr);
    // sidecde.draw(gr);
    // highLightPoint(gr,p1,6,3); //green
    // highLightPoint(gr,p2,6,4); //blue
    // highLightPoint(gr,p3,6,2); //red
    // highLightPoint(gr,p4,6,0); //black
    // // //cout << gr.toString();
    // gr.toPPM();
    return dparr;
}
...
{
    // Read the file
    fstream myFile("pointsEx.txt", ios::in);
    ...
    // calc with dir = 1
    // calc with dir = -1
    vector<DoublePoint> *minPointArr = NULL;
    ...
    for (int i = 1; i < 4; i++)
    {
        ...
        for (int dir:possDir)
        {
            vector<DoublePoint> currArr = calculateLines(myPoints[a], myPoints[b], myPoints[c], myPoints[d], dir);
            ...
            outputFile << fixed << " Area = " << area << endl;
            if (area < minArea)
            {
                delete minPointArr;
                minPointArr = new vector<DoublePoint>();
