...
// PART 3
MergeReturnObj zipperMergePt3(vector<DoublePoint> &points, int start, int mid, int end,double d)
{
    double minDist = DBL_MAX;
    int mindp1 = 0;
    int mindp2 = 0;
    ...
    //Read into vector
    int size=(int)vectorStrip.size();
    for (int p1 = 0; p1 < size; p1++)
    {   
        int p2lim = min(p1+15,size);
        //cout << "is lim p1+8"<<(p2lim==p1+8)<<"\n";
        //cout << p2lim == vectorStrip.size();
...
MergeReturnObj mergePt3(vector<DoublePoint> &points, int start, int end)
{
    ...
    else
    {
        ...
        // cout << "l="<<l<<" r="<<r<< " count "<< currCount<<endl;
        MergeReturnObj strip = zipperMerge(points,l,midpoint,r);//zipperMergePt3(points, l, midpoint, r,smallest);
        // count += 1;
        if (strip.getDistance() < smallest)
        {
            return strip;
...
string part3()
{
    ...
    // for(DoublePoint i:myPoints)
    //     cout << "Point"<<i << endl;
    MergeReturnObj mergeRet = mergePt3(myPoints, 0, myPoints.size() - 1);
    auto stop = high_resolution_clock::now();
    auto duration = duration_cast<microseconds>(stop - start);
    stringstream stream;
    stream.precision(PRECISION);
