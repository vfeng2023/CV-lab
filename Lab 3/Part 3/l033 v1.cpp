...
list<DoublePoint> split(string &filename)
{
    ...
    while (getline(myFile, line))
    {
        ...
        double x = stod(coords[0]);//*100;
        double y = stod(coords[1]); //*100;
        DoublePoint dp(x, y);
        toRet.push_back(dp);
    }
    return toRet;
}
void splitpt2(string &filename, vector<DoublePoint>& toRet)
{
    fstream myFile;
    myFile.open(filename);
...
MergeReturnObj zipperPt3(vector<DoublePoint> &strip,double currMin){
    double minDist = currMin;
    int i = 0;
    int j = 0;
    int midpt = (strip.size()-1)/2;
    //cout << "Size of strip: "<<strip.size()<<endl;
    
    for(size_t p1=0; p1 < strip.size(); p1++){
    //for(size_t p1=0; p1 < midpt; p1++){
        //int p2upper = min(p1+7,strip.size());
        //int p2upper = strip.size();
        for(size_t p2 = p1+1; p2 < strip.size() && strip[p2].getY() - strip[p1].getY() < currMin;p2++){ //&& p2 <= p1+15; p2++){
            //if(p1!=p2){ 
                double d = Line::calcDist(strip[p1],strip[p2]);
                if(d < minDist){
                    minDist = d;
                    i = p1;
                    j = p2;
    ...
    return MergeReturnObj(minDist,strip[i],strip[j]);
}
MergeReturnObj mergePt3(vector<DoublePoint>& points, int start, int end)
{
    ...
    vectorStrip.push_back(points[midpoint]);
    // brute force on the strip
    int l = midpoint-1;
    // while(l > start && abs(points[l].getX()-points[midpoint].getX()) <= smallest){
    while (l > start && points[l].getX() > lbound)
    {   
        vectorStrip.push_back(points[l]);
        l--;
    }
    int r = midpoint+1;
    // while( r < end && abs(points[r].getX()-points[midpoint].getX()) <= smallest){
    while (r < end && points[r].getX() < rbound)
    {   
        vectorStrip.push_back(points[r]);
        r++;
    }
    // get vectorStrip
    // vector<DoublePoint> vectorStrip;
    // for(int i=start; i <=end;i++){
    //     if(abs(points[i].getX()-points[midpoint].getX()) < smallest){
    ...
    if(vectorStrip.size() >=2){
        std::sort(vectorStrip.begin(), vectorStrip.end(), DoublePoint::comparePointY);
        MergeReturnObj strip = zipperPt3(vectorStrip,smallest);
        //MergeReturnObj strip= zipperMerge(vectorStrip,0,vectorStrip.size()/2, vectorStrip.size()-1);
        // count += 1;
        if (strip.getDistance() < smallest)
        {
            return strip;
...
string part3()
{
    ...
    splitpt2(filename, points);
    auto start = high_resolution_clock::now();
    std::sort(points.begin(),points.end(),DoublePoint::comparePoint);
    MergeReturnObj result = mergePt3(points,0,points.size()-1);
    auto end = high_resolution_clock::now();
    auto duration = duration_cast<microseconds>(end-start);
    //print results
    stringstream stream;
    ...
    stream << "Time taken: " << duration.count() << " microseconds " << endl;
    // double distance = mergeRet.getDistance();
    DoublePoint dp1 = result.getPoint1();
    DoublePoint dp2 = result.getPoint2();

    stream << "Point 1: " << dp1.toString(true) << endl;
    stream << "Point 2: " << dp2.toString(true) << endl;
    stream << "Minimum Distance: " << result.getDistance() << endl;
    return stream.str();
