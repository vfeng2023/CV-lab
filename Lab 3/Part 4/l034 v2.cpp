...
// PART 3
MergeReturnObj zipperMerge3(vector<DoublePoint> &points, int start, int mid, int end,double smallest)
{
    double minDist = DBL_MAX;
    int mindp1 = 0;
    int mindp2 = 0;
    ...
    // {
    //     if(abs(points[k].getX() - points[mid].getX()) < smallest){
...
MergeReturnObj merge3(vector<DoublePoint> &points, int start, int end)
{
    ...
    else
    {
        ...
        // cout << "l="<<l<<" r="<<r<< " count "<< currCount<<endl;
        MergeReturnObj strip = zipperMerge3(points, l, midpoint, r,smallest);
        // count += 1;
        if (strip.getDistance() < smallest)
        {
            return strip;
