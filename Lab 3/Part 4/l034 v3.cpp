...
public:
    ...
    */
    ...
    double static calcDist2(DoublePoint d1, DoublePoint d2){
        double t1 = pow(d1.getX() - d2.getX(),2);
        double t2 = pow(d1.getY() - d2.getY(),2);
        double dist2 = t1 + t2;
        return sqrt(dist2);
    }
    void draw(Grid &gr)
...
// PART 3
MergeReturnObj zipperMerge3(vector<DoublePoint> &vectorStrip, int start, int mid, int end,double smallest)
{
    double minDist = DBL_MAX;
    int mindp1 = 0;
    int mindp2 = 0;
    ...
    // {   
    //     if ((points[k].getX() - points[mid].getX()) < smallest)
    ...
    for (size_t i = 0; i < vectorStrip.size(); i++)
    {
        ...
        for (size_t j = i + 1; j < upper; j++)
        {
            ...
            double dist = Line::calcDist2(p1, p2);
            if (dist < minDist)
            {
                mindp1 = i;
                mindp2 = j;
    ...
    return MergeReturnObj(minDist, vectorStrip[mindp1], vectorStrip[mindp2]);
}
MergeReturnObj merge3(vector<DoublePoint> &points, int start, int end)
{
    ...
    else
    {
        ...
        for (int k = l; k <= r; k++)
        {   
            // if ((points[k].getX() - points[midpoint].getX()) < smallest)
                vectorStrip.push_back(points[k]);
        }
        //cout << vectorStrip.size() << endl;
        MergeReturnObj strip = zipperMerge3(vectorStrip, l, midpoint, r,smallest);
        // count += 1;
        if (strip.getDistance() < smallest)
        {
            return strip;
...
public:
    size_t operator()(const Box &b) const
    {
        ...
        return hash<unsigned long long>()(b.x) ^ (hash<unsigned long long>()(b.y) << 1);
    }
};
...
MergeReturnObj randomizedAlgorithm(vector<DoublePoint> &points)
{
    ...
    for (size_t i = 2; i < points.size(); i++)
    {
        ...
        unsigned long long startRow = myBox.x - 2;
        if (myBox.x < 2)
        {
            startRow = 0;
        }
        unsigned long long startCol = myBox.y - 2;
        if (myBox.y < 2)
        {
            startCol = 0;
        }
        ...
        for (unsigned long long r = startRow; r <= myBox.x + 2; r++)
        {
            for (unsigned long long c = startCol; c <= myBox.y + 2; c++)
            {
                Box key(r, c);
                count += 1;
