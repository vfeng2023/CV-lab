...
public:
    ...
    IntegerPoint(IntegerPoint &&ip){
        x = ip.getX();
        y = ip.getY();
    }
    IntegerPoint(const IntegerPoint &ip){
        x = ip.getX();
        y = ip.getY();
    }
    string toString(bool fullPrecision = false) const
    {
        if (!fullPrecision)
        {
...
void draw(string filename){
    ...
    for(DoublePoint& dp:points){
        IntegerPoint p = dp.scaleToInt(800);
        gr.drawPixel(p);
        //highLightPoint(gr,dp,3);
    }
    gr.toPPM(filename.substr(0,filename.size()-4)+".ppm");
}
int main()
{
