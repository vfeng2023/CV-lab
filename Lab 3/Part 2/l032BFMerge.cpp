...
public:
    ...
    DoublePoint& operator=(DoublePoint &&rhs){
        // x = rhs.getX();
        // y = rhs.getY();
        if(this!= &rhs){
            this->x = rhs.x;
    ...
    // DoublePoint& operator=(DoublePoint dp){
    //     this->x = dp.x;
    //     this->y = dp.y;
    //     return *this;
    // }
    ...
    void drawLine(DoublePoint &d1, DoublePoint &d2)
    {
        ...
        if(yx0 >= 0 && yx0 <=1){
            if(!chosestart){
                //start.setX(0);
                //start.setY(yx0);
                start = DoublePoint(0,yx0);
                chosestart = true;
            }else{
                // end.setX(0);
                // end.setY(yx0);
                end = DoublePoint(0,yx0);
            }
        ...
        if(yx1 >=0 && yx1 <=1){
            if(!chosestart){
                // start.setX(1);
                // start.setY(yx1);
                start = DoublePoint(1,yx1);
                chosestart=true;
            }else{
                // end.setX(1);
                // end.setY(yx1);
                end = DoublePoint(1,yx1);
            }
        }
        if(xy0 >=0 && xy0 <=1){
            if(!chosestart){
                ...
                // start.setX(xy0);
                // start.setY(0);
                start = DoublePoint(xy0,0);
                chosestart =true;
            }else{
                // end.setX(xy0);
                // end.setY(0);
                end = DoublePoint(xy0,0);
        ...
        if(xy1 >=0 && xy1 <=1){
            if(!chosestart){
                // start.setX(xy1);
                // start.setY(1.0);
                start = DoublePoint(xy1,0);
                chosestart=true;
            }else{
                // end.setX(xy1);
                // end.setY(1.0);
                end = DoublePoint(xy1,1);
            }
        }
        cout << "\n"<<start.toString()<<endl;
...
void part1(){
    ...
    cout << "Brute force method (Part 1)"<<endl;
...
MergeReturnObj merge(vector<DoublePoint> &points,int start,int end){
    if(end-start+1 <= 3){
        ...
        //cout << "1009 merge function start and end: " << start <<" "<<end<< "level"<<count<<endl; 
        //cout << "Hello"<<endl;
        ...
        //cout << "line 1034\n";
        MergeReturnObj strip = baseCaseMerge(points,l,r);
         count += 1;
