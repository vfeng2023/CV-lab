...
/**/
void cannyEdge(Mat& image, Mat& output, int low, int high,bool ishard,bool noFilter) {
    Mat blurredimage;
    ...
    if (!ishard) {
        ...
        //dilate(image, image, getStructuringElement(MORPH_RECT, Size(3, 3)), Point(-1, -1), 10);
        //erode(image, image, getStructuringElement(MORPH_RECT, Size(3, 3)), Point(-1, -1), 10);
...
void findCircles(Mat& grayImage, vector<Vec3f>& circles, double highThreshold, double minRad, double maxRad, double thresholdCount, double minDist,bool ishard) {
    ...
    if (!ishard) {
        ...
        HoughCircles(grayImage, candidates, HOUGH_GRADIENT, 1, minDist, highThreshold,thresholdCount, minRad, maxRad);
...
void identifyCoins(Mat& colorImg, bool& ishard, vector<Vec3f>& circleCandidates, vector<int>& identified) {
    ...
    cout << "Average penny size: " << avgPenny << endl;
    if (abs(avgPenny) < 1) {
        avgPenny = 87;
    }
       ...
       */
       //if computed ratio falls within 0.05 of reference ratio, classify as said coin
        ...
        for (size_t idx = 0; idx < circleCandidates.size(); idx++) {
            ...
            else if (abs(ratio - pennyratio) < 0.07) {
                /*if (abs(ratio - pennyratio) < 0.05) {
                    Vec3b hsvPixel = adjusted.at<Vec3b>(Point(x,y));
                    int hue = hsvPixel[0];
                    if (hue >= 16 && hue <= 25|| totalArea > 0.25 * area) {
                        identified.push_back(0);
                    }
                    else {
            ...
            else if (abs(ratio - nickelratio) < 0.1) {

                if (totalArea > 0.25 * area) {
                    identified.push_back(0);
                }
                else {
                    double qerr = abs(ratio - quarterratio);
                    double nerr = abs(ratio - nickelratio);
                    double minErr = min(qerr,minErr);
                    if (qerr < minErr) {
            ...
            else if (abs(ratio - quarterratio) < 0.05) {
        ...
        for (size_t idx = 0; idx < circleCandidates.size(); idx++) {
            ...
            else if (abs(ratio - golddolar) < tolerance) {
                identified.push_back(5);

            }
            else if(abs(ratio - silverdollar) < 0.1){
                identified.push_back(5);
            }
            else {
                identified.push_back(-1);
...
void part3(int argc, char** argv)
{
    ...
    */
    ...
    int maxRad = 120;
    bool noFilter = true;
    -f coinsEasy.jpg -ht 150 -TC 27 -mincentdist 80 -minR 80 -maxR 120 -nofilter -fg easyg.jpg -fCi easycircles.jpg -fCo easyCoins.jpg -fR easyresults.txt
    */
    /*
    ...
    int minRad = 80;
    int maxRad = 120;
    -f coinsHarder.jpg -ht 200 -TC 25 -mincentdist 50 -minR 80 -maxR 120 -fg medg.jpg -fCi medcircles.jpg -fCo medCoins.jpg -fR medresults.txt -ff medf.jpg
    */
    /*
    Hard image values:
    int high = 300;
    ...
    */
    ...
    //bool thresholdOverride = false;
    bool hardimage = false;
    int minCenterDist = 100;
    int minRad = 80;
    int maxRad = 120;
    bool noFilter = false;
    //hard image:
    /* -f coinsHardest.jpg -ht 105 -TC 50 -hard -minR 70 -maxR 180 -mincentdist 100 -fg hardg.jpg -ff hardf.jpg -fCi hardcircles.jpg -fCo hardCoins.jpg -fR hardresults.txt*/
    ...
    cout << "\n\nCommand line args: " << endl;
    cout << "-f " << imageName << " -ht " << high << "-TC " << thresholdCount << " -fCi " << fci << " -fCo " << fco
        << " -fR " << fr << " -mincentdist " << minCenterDist << " -minR " << minRad << " -maxR " << maxRad;
    if (hardimage) {
        cout << " -hard \n";
    }
    if (noFilter) {
   ...
   /* Mat highContrast;
    ...
    //    blur(imageg, imageg, Size(12, 12));
    //}
    

    Mat edges;
    //apply canny edge
    cannyEdge(imageg, edges, low, high,hardimage,noFilter);
    ...
    for (size_t i = 0; i < circles.size(); i++)
    {
        ...
        //if (coinTypes[i] != -1) {
            circle(image, center, 4, Scalar(0, 0, 255), -1, 8, 0);
    ...
    for (size_t i = 0; i < coinTypes.size(); i++) {
        ...
        int radius = cvRound(circles[i][2]);
        if (coinTypes[i] == 0) {
            circle(image, center, radius, Scalar(0, 0, 255), 4, 8);
        }
        else if (coinTypes[i] == 1) {
            circle(image, center, radius, Scalar(0, 255, 255), 4, 8);
        }
        else if (coinTypes[i] == 2) {
            circle(image, center, radius, Scalar(255, 0, 0), 4, 8);
        }
        else if (coinTypes[i] == 3) {
            circle(image, center, radius, Scalar(128, 0, 128), 4, 8);
        }
        else if (coinTypes[i] == 4) {
            circle(image, center, radius, Scalar(0, 255, 0), 4, 8);
        }
        else if (coinTypes[i] == 5) {
            circle(image, center, radius, Scalar(128, 0, 255), 4, 8);
        }
        else {
            circle(image, center, radius, Scalar(255, 255, 255), 4, 8);
        }
