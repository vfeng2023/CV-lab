 ...
 * 
 */
vector<DoublePoint> grahamScan(vector<DoublePoint> &points){
    // let points be the list of points
    // let stack = empty_stack()
    vector<DoublePoint> stk;
    ...
    std::sort(points.begin(),points.end(),GrahamScanComparer(p0));
    ...
    for(size_t j=0; j < points.size();j++){
        ...
        while(stk.size() > 1 && isRight(stk[stk.size()-2],stk[stk.size()-1],points[j])){
            stk.pop_back();
        }
        stk.push_back(points[j]);
...
void part2(){
    generatePoints(60);
    vector<DoublePoint> points = splitpt2("points.txt");
    vector<DoublePoint> hull = grahamScan(points);
    ...
    for(int j=hull.size()-1; j > 0;j--){
        ...
        highLightPoint(gr,hull[j],4,2);
        IntegerPoint p1 = hull[j].scaleToInt(gr.getSize());
        IntegerPoint p2 = hull[j-1].scaleToInt(gr.getSize());
        gr.drawLine(p1,p2);
    }
    gr.toPPM("quickhull2.ppm");
