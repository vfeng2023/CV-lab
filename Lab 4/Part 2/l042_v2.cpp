...
stack<DoublePoint> grahamScan(vector<DoublePoint> &points)
{
    ...
    // vector<DoublePoint> visitedPoints;
    // eliminate duplicates
    // for(int j=0; j < points.size();j++){
    //     if(j==idx){
    //         continue;
    //     }else if(j > 0){
    //         double jcos = comp.calcCos(points[j]);
    //         double prevCos = comp.calcCos(points[j-1]);
    //         if(abs(jcos-prevCos) < 1e-15){

    //         }
    //     }
    // }
    // vector<DoublePoint> newPoints;
    // for(p in points)
    ...
    for (size_t j = 2; j < points.size(); j++)
    {
        ...
        while (stk.size() > 1 && isRight(A, B, points[j]))
        {
            B = stk.top();
            stk.pop();
            A = stk.top();
