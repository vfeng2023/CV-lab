...
void findHull(vector<DoublePoint> &points,vector<int> &subset,vector<int> &poly, int idx1, int idx2){
    // if no points return 
    if(subset.size()==0){
        return;
    }
    ...
    double minDist = calcDistfromLine(points[aidx],points[bidx],points[smallestIdx]);
    for(int idx:subset){
        double currDist = calcDistfromLine(points[aidx],points[bidx],points[smallestIdx]);
        if(currDist < minDist){
            smallestIdx = idx;
            minDist = currDist;
        }
    ...
    //insert between ab
    poly.insert(poly.begin()+idx2,smallestIdx);
    //call findhull again
    vector<int> s1;
    vector<int> s2;
    ...
    findHull(points,s1,poly,idx2-1,idx2);
    findHull(points, s2,poly,idx2,idx2+1);
}
/*
    Precondition: points is nonzero, hull is empty
*/
vector<int> quickHull(vector<DoublePoint> &points){
    ...
    vector<int> polygon={aidx,bidx,aidx};
    //define s1 and s2
    vector<int> s1;
    vector<int> s2;
    ...
    findHull(points,s1,polygon,0,1);
    findHull(points,s2,polygon,polygon.size()-2,polygon.size()-1);
    return polygon;
...
void part1(){
    ...
    // vector<int> hull;
    vector<int> hull = quickHull(points);
    // quickHull()
    //quickHull(points);
    //  e) create a ppm of size 400x400 named quickhull.ppm in which you display a circle of radius 3 for each scaled point you generated, as well as connect the vertices you obtained for the convex hull (so in the image I should see 60 points and a convex hull with vertices circles of radus 3 that are conected forming a convex hull)
    Grid gr(400);
    ...
    highLightPoint(gr,points[0],4,2);
...
int main(){
    // DoublePoint a = DoublePoint(0,0);
    // DoublePoint b = DoublePoint(1,1);
    // DoublePoint c = DoublePoint(0,1);
    // double dist = calcDistfromLine(a,b,c);
    // cout << dist;
    part1();
}